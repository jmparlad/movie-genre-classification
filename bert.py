...
from torch.nn import CrossEntropyLoss, DataParallel, Module, Linear, Sigmoid
...
class BertClassifier(ABC):
    ...
    def __init__(
        ...
        num_labels: Optional[int] = 2 ### ADDED
    ):
        if not tokenizer:
    ...
    def predict(self, x: list[str], batch_size: Optional[int] = None) -> list[tuple[bool, float]]:### MODIFIED
        if not batch_size:
            batch_size = self.batch_size
        scores = self.predict_scores(x, batch_size)
        classes = []
        for i in range(0, len(scores), self.num_labels):
            iter_scores = scores[i:i + self.num_labels]
            classes.append(max(range(self.num_labels), key=iter_scores.__getitem__))
        return list(zip(classes, scores))

    def predict_classes(self, x: list[str], batch_size: Optional[int] = None) -> list[bool]:### MODIFIED
        if not batch_size:
            batch_size = self.batch_size
        scores = self.predict_scores(x, batch_size)
        classes = []
    ...
    def _train_single_epoch(self, dataloader: DataLoader, optimizer: Optimizer) -> None: ### MODIFIED
        ...
        for step, batch in enumerate(dataloader):
            optimizer.zero_grad()
            labels = batch[-1].cpu()
            predictions = torch.reshape(self._evaluate_single_batch(batch), (self.batch_size, self.num_labels))
...
class BertClassifierNN(Module):### MODIFIED
    def __init__(self, model: Union[BertModel, RobertaModel], num_labels):
        super().__init__()
        self.model = model
